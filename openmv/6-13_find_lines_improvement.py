...
#distance settings
ROI = (0, 50, 160, 20)
red_threshold_LAB=((41, 72, 2, 66, 2, 80))#in LAB color space for finding blobs
...
while(True):
    ...
    if blobs:
        ...
        for i in range(len())
                if abs(lines[0].theta()-lines[1].theta())<10:
                        if abs(lines[0].rho()-lines[1].rho())>20:
                            rho_center= (lines[0].rho() + lines[1].rho())/2
                            theta_center = (lines[0].theta()+lines[1].theta())/2
        ...
        ##6.12 改动这里 只允许输出两条的情况出现 取theta相近的
